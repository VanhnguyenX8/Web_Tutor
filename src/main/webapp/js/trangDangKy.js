	...
	uniqueKhoiValues.forEach(uniKhoi => {
		...
		datas.forEach(data => {
			if(data.khoi === uniKhoi){
				uniqueMonValues.add(data.ten_lop_hoc);
			}
		})
		uniqueMonValues.forEach(uniMon => {
			...
			datas.forEach(data => {
				if(data.khoi == uniKhoi && data.ten_lop_hoc == uniMon && data.username_hoc_sinh == null){
					...
					liLop.innerHTML = `
						...
						<div class="thoiGian">${data.gio_hoc}:00, ${data.ngay_hoc}</div>
						<div class="hocPhi">${data.hoc_phi}đ</div>
...
async function disableChonbtn(){
	...
	var dataArray = [];
	datas.forEach(data => {
		dataArray.push(data.gio_hoc + ":00, " + data.ngay_hoc);
	})
